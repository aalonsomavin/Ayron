...
from apps.agent.tools.table_style_tokens import (
    cell_preview_fill_classes,
    fill_preview_class,
    resolve_border_color,
    resolve_cell_fill_token,
    resolve_fill_hex,
    resolve_font_color,
)
...
def _border_for_row(row_style: str) -> Border:
    color = resolve_border_color(row_style)
    side = Side(style="thin", color=color)
    return Border(left=side, right=side, top=side, bottom=side)


def _pattern_fill(fill_token: str) -> PatternFill:
    return PatternFill(
        fill_type="solid",
        start_color=resolve_fill_hex(fill_token),
        end_color=resolve_fill_hex(fill_token),
    )
...
def _apply_cell_style(
    ...
    *,
    header: bool = False,
    row_style: str = "default",
    row_idx: int = 0,
    sheet_striped: bool = True,
    header_fill: str = "muted",
) -> None:
    if header:
        fill_token = header_fill
        bold = True
        tone = "default"
        align = cell_data.get("align", "left")
    else:
        fill_token = resolve_cell_fill_token(
            cell_data,
            row_style=row_style,
            row_idx=row_idx,
            sheet_striped=sheet_striped,
        )
        bold = cell_data.get("bold", False)
        tone = cell_data.get("tone", "default")
        align = cell_data.get("align", "left")

    font_color = resolve_font_color(tone, header=header, fill_token=fill_token)
    cell.font = Font(bold=bold, color=font_color)
    cell.alignment = Alignment(horizontal=ALIGN_MAP.get(align, "left"))
    cell.fill = _pattern_fill(fill_token)
    cell.border = _border_for_row(row_style if not header else "default")


def _write_sheet(ws, sheet_data: dict) -> None:
    headers = sheet_data.get("headers", [])
    sheet_style = sheet_data.get("style", {})
    sheet_striped = sheet_style.get("striped", True)
    header_fill = sheet_style.get("header_fill", "muted")
    ...
    for row_idx, row in enumerate(sheet_data.get("rows", [])):
        cells = row.get("cells", [])
...
def _cell_classes(
    ...
    *,
    ...
    if header:
        ...
        header_cls = fill_preview_class(header_fill)
        if header_cls:
            classes.append(header_cls)
    else:
        align = cell_data.get("align", "left")
        if align != "left":
...
def _render_sheet_html(sheet_data: dict, *, active: bool) -> str:
    ...
    for row_idx, row in enumerate(rows, start=2):
        ...
        for col_idx in range(col_count):
            ...
            parts.append(
                f'<div class="{_cell_classes(cell_data, row_style=row_style, row_idx=data_row_idx, sheet_striped=sheet_striped)}">'
...
def create_spreadsheet(
    ...
    Each sheet has a `name`, `headers`, `rows`, and optional `style` (`striped`,
    `header_fill`). Rows can be plain lists or objects with `cells` and optional
    `style` (total, subtotal). Cells support `value`, `align`, `tone` (text color:
    success, danger, warning, muted), `fill` (background: muted, subtle, accent_light,
